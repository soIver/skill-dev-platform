...
from .utils import get_level_index_normal, calculate_adjusted_score, calculate_confidence, calculate_vtotal
...
async def search_skill_levels(
    ...
    items = []
    for row in rows:
        obtained = await _count_obtained(db, row.skill_id, row.order_index)
...
async def _count_obtained(db: AsyncSession, skill_id: int, target_order: int) -> int:
    """
    Подсчёт уникальных пользователей, достигших данного уровня навыка.
    Учитываются RepoSkill (через get_level_index_normal) и TestAttempt (score >= threshold).
    target_order — 1-based order_index из SkillLevel
    """
    # количество уровней для навыка
    num_levels = await db.scalar(
        select(func.count(SkillLevel.id)).where(SkillLevel.skill_id == skill_id)
    )
    if not num_levels:
        return 0

    # переводим target_order (1-based) в 0-based для сравнения с get_level_index_normal
    target_index = target_order - 1

    user_ids = set()

    # 1. пользователи из repo_skills
    repo_query = (
    ...
    # 2. пользователи из test_attempts (score >= threshold)
    sl_ids_query = (
...
async def create_skill_level(
    ...
    if existing_obj:
        obtained = await _count_obtained(db, skill_obj.id, existing_obj.order_index)
...
async def get_skill_level(
    ...
    sl = await db.get(SkillLevel, sl_id)
    if not sl:
        raise HTTPException(status_code=404, detail="SkillLevel not found")

    skill = await db.get(Skill, sl.skill_id)
    ...
    # связи, где target_id = skill_id (навыки, которые влияют на данный)
...
async def update_skill_level(
    ...
    await db.commit()
    return await get_skill_level(sl_id, db, claims)
...
async def delete_skill_level(
    ...
    """удаление компетенции; блокируется если привязан тест"""
    sl = await db.get(SkillLevel, sl_id)
    if not sl:
        raise HTTPException(status_code=404, detail="SkillLevel not found")
    ...
    # пересчёт order_index для оставшихся уровней
...
async def get_my_skills(
    ...
    """
    Список навыков текущего пользователя, вычисленный на лету
    из RepoSkill и TestAttempt
    """
    # все уникальные skill_id из repo_skills пользователя
    skill_ids_query = (
        select(RepoSkill.skill_id)
        .join(UserRepo, RepoSkill.repo_id == UserRepo.id)
        .where(UserRepo.user_id == user.user_id, RepoSkill.skill_id.isnot(None))
        .distinct()
    )
    skill_ids_result = await db.execute(skill_ids_query)
    repo_skill_ids = {row[0] for row in skill_ids_result.all()}

    # skill_id из успешных тестовых попыток
    ...
    test_skill_ids_result = await db.execute(test_skill_ids_query)
    ...
    for skill_id in page_skill_ids:
        item = await _compute_user_skill(db, user.user_id, skill_id)
        if item:
            items.append(item)
...
async def _compute_user_skill(
    db: AsyncSession, user_id: int, skill_id: int,
) -> UserSkillItem | None:
    """
    Вычисление навыка пользователя на лету:
    определение уровня через get_level_index_normal + расчёт уверенности
    """
    # уровни навыка
    levels_result = await db.execute(
        select(SkillLevel)
    ...
    # средний балл за последние SKILL_SCORE_DECAY_MAX_DAYS дней с учётом time decay
    ...
    # применение Vtotal: корректировка средней оценки по связям навыков
    # (читается при каждом запросе, т.к. связи могут меняться)
    relations_result = await db.execute(
    ...
    if relations:
        ...
        src_scores_result = await db.execute(
    ...
    current_level = await db.get(Level, current_prof.level_id)
    ...
    # штраф за малое количество встреч навыка
    n_query = select(func.count(RepoSkill.id)).join(UserRepo).where(
        UserRepo.user_id == user_id,
        RepoSkill.skill_id == skill_id,
    )
    n = await db.scalar(n_query) or 0
